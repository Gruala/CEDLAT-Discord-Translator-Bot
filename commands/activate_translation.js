...
const {                                           //Traducción    /     Translation
  ...
  Command_activate_translation_0,
  Command_activate_translation_1,
  Command_activate_translation_2,
} = require("../languages/Config_Language.js");   //Traducción    /     Translation

module.exports = {
  name: `${Command_activate_translation_0}`,
  cooldown: 3,
  aliases: [`${Command_activate_translation_1}`],
  description: `${Command_activate_translation_2}`,
  execute(message, args, client, GENERAL_ENABLED) {
    ...
    if (user_boolean.admin_user ) {         //Returns Boolean: Is it administrator? (True or False   /      Devuelve booleano: ¿es administrador? (True o False
      ...
      if (GENERAL_ENABLED.trans_enabled) {
        GENERAL_ENABLED.trans_enabled= false
        translator_text1 =`${Command_activate_command_0} ${Command_activate_translation_0}`
        translator_text2 =`${Command_activate_command_1}`
      } else {
        GENERAL_ENABLED.trans_enabled= true
        translator_text1 =`${Command_activate_command_2} ${Command_activate_translation_0}`
      ...
      var obj = {
        ...
        if(exists){
          ...
          fs.writeFile( "./translate/config_translate.json", json, 'utf8', function (error) {
            ...
            return message
              .reply(`${translator_text1}\n ${Command_activate_translation_10} ${translator_text2}.\n${translator_text3}`)
              .catch(console.error);
          });
        }
      });
